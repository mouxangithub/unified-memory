...
/**
 * AgentBrain 简化版 MCP 服务器
 * 使用简单文件存储，快速集成到 OpenClaw
 */

import { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { StdioServerTransport } from '@modelcontextprotocol/sdk/server/stdio.js';
import { SimpleAgentBrain } from './agent_brain_simple.js';
import { z } from 'zod';

// 创建 MCP 服务器
const server = new McpServer({
  name: 'agent-brain-simple',
  version: '1.0.0',
  description: 'AI Agent 记忆大脑 - 简化文件存储版'
});

// 初始化 AgentBrain
const brain = new SimpleAgentBrain();
...
async function main() {
  ...
  console.error('🧠 AgentBrain 简化版 MCP 服务器已启动');
  console.error('📁 存储目录: /root/.openclaw/workspace/agentbrain_storage');
}

...
def _get_all_capabilities() -> Dict:
    """获取所有能力"""
    return {
        "persistent_context": True,           # 持久化上下文
        "incremental_learning": True,          # 增量学习
        "proactive_recall": True,              # 主动回忆
        "multi_agent_share": True,             # 多智能体共享
        "crossmodal": True,                    # 跨模态
        "smart_compress": True,                # 智能压缩
        "bm25": True,                           # BM25
        "hybrid_search": True,                 # 混合搜索
        "feedback": True,                      # 反馈学习
        "smart_fallback": True,               # 智能降级
    }


def get_system_status() -> Dict:
    """获取系统状态"""
    from memory_temporal import load_memories
    from memory_persistent_context import PersistentContext
    from memory_incremental_learning import IncrementalLearner
    from memory_crossmodal import CrossModalEngine
    ...
    pc_info = {
        "has_summary": bool(pc.context.get("summary")),
        "recent_count": len(pc.context.get("recent", [])),
        "working_keys": list(pc.context.get("working", {}).keys())
    }
    ...
    # 跨模态
    crossmodal = CrossModalEngine()
    crossmodal_stats = crossmodal.get_stats()
    
    return {
        "total_memories": len(memories),
        "capabilities": _get_all_capabilities(),
        "persistent_context": pc_info,
        "incremental_learning": learner_stats,
        "crossmodal": crossmodal_stats,
    }
